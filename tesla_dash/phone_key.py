...
<body>
  <div class="wrap">
    ...
    <div class="card" id="ios-panel" hidden>
      ...
      <a class="btn" href="/downloads/PulsePhoneKey-playground-build10.zip" style="text-decoration:none;text-align:center">Playgrounds build-10 (Pair + HUD düzgün)</a>
      ...
      <p class="support" style="background:rgba(255,214,10,.12);color:#ffe566;margin-top:.75rem">
        <strong>SÜRÜM: build-10</strong> · Tüm BLE listesi → 🔑 Tesla’ya dokun → kart → HUD otomatik.
        PIN <strong>428462</strong>.
      </p>
...
def register_phone_key(server) -> None:
    ...
    def download_playgrounds_build8_zip():  # type: ignore[no-redef]
        return _send_release_zip("PulsePhoneKey-playground-build9.zip")

    @server.get("/downloads/PulsePhoneKey-playground-build9.zip")
    def download_playgrounds_build9_zip():  # type: ignore[no-redef]
        return _send_release_zip("PulsePhoneKey-playground-build10.zip")
