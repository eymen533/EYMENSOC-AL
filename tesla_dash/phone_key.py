  ...
  <div class="wrap">
    ...
    <div class="card" id="ios-panel" hidden>
      ...
      <ol style="margin:0 0 .9rem;padding-left:1.15rem;color:rgba(255,255,255,.88);font-size:.88rem;line-height:1.45">
        ...
        <li>Signing → <strong>Run ▶</strong> → sürüm <code>build-35-crashfix</code></li>
        <li>Pair Vehicle → Key Card konsola → <strong>Cluster’i ac</strong> (uygulama içi · yatay)</li>
      </ol>
      <a class="btn" href="/downloads/PulsePhoneKey-playground-build35.zip" style="text-decoration:none;text-align:center">iPhone · build-35 crashfix</a>
      ...
      <p class="support" style="background:rgba(255,214,10,.12);color:#ffe566;margin-top:.75rem">
        <strong>SÜRÜM: build-35-crashfix</strong> · 5’li slider · çökme düzeltmesi · BLE telemetri
      </p>
...
def register_phone_key(server) -> None:
    ...
    def iphone_public_download_page():  # type: ignore[no-redef]
        ...
        gh = (
            ...
            "releases/PulsePhoneKey-playground-build35.zip"
        )
        blob = (
...
<h1>PulsePhoneKey · build-35</h1>
<p>PIN yok. Asagidan <strong>birini</strong> dene. Indir → Dosyalar → <code>.swiftpm</code> basili tut → Playgrounds → Run ▶ → ekranda <code>build-35-crashfix</code></p>
...
<p class="ok">Safari bazen zip’i acamaz: Dosyalar uygulamasinda gorunur. Playgrounds’ta eski projeyi sil, yeniyi ac.</p>
    ...
    def download_iphone_short_zip():  # type: ignore[no-redef]
        return _send_release_zip("PulsePhoneKey-playground-build35.zip", public=True)
